...
VolcanoPlot2<- EnhancedVolcano (Complexes_Prot,
                lab = Complexes_Prot$Symbol,#Metabolite name
                x = "log2FoldChange",#Log2FC
                y = "pval",#p-value or q-value
                xlab = bquote(~Log[2]~ "FC"),
...
ggsave("Figures/Proteomics/VolcanoPlot_mt-Complexes_TGvWT_p.val.pdf", plot=VolcanoPlot2, width=10, height=8)
plot(VolcanoPlot2)
...
ggsave("Figures/Proteomics/Emapplot_Log2FCbg0.5_padj0.05_TGvWT_Top40.pdf", plot=Emapplot1, width=10, height=8)
...
\
Lastly, it is worth mentioning that there are ties in the preranked stats (2.63% of the list). The order of those tied genes will be arbitrary, and hence we will get slightly different results in every rerun. 
```{r, message=FALSE, warning=FALSE}
#Install and download the package
library(reticulate)
#py_install("scibiomart", pip = TRUE)# install scibiomart
...
```{r, warning= FALSE, message=FALSE}
#Establish function
DoVolcano_1 <- function(InputData, OutputfileName){
  gsea_result1 <- InputData
  ##Plot
  ...
  VolcanoPlot1 <- EnhancedVolcano (Volcano1,
                ...
                xlab = "NES",
                ylab = bquote(~-Log[10]~p.adj),#(~-Log[10]~adjusted~italic(P))
                selectLab = c("KEGG_OXIDATIVE_PHOSPHORYLATION","HALLMARK_OXIDATIVE_PHOSPHORYLATION","REACTOME_RESPIRATORY_ELECTRON_TRANSPORT","REACTOME_TCA_CYCLE_AND_RESPIRATORY_ELECTRON_TRANSPORT", "HALLMARK_UNFOLDED_PROTEIN_RESPONSE", "REACTOME_MITOCHONDRIAL_PROTEIN_IMPORT", "ATF4_Han", "BIOCARTA_EIF_PATHWAY", "BIOCARTA_EIF2_PATHWAY", "HALLMARK_FATTY_ACID_METABOLISM", "REACTOME_SYNTHESIS_OF_VERY_LONG_CHAIN_FATTY_ACYL_COAS", "HALLMARK_ADIPOGENESIS", "REACTOME_ION_TRANSPORT_BY_P_TYPE_ATPASES", "HALLMARK_MTORC1_SIGNALING"),
                pCutoff = 0.25,
                FCcutoff = 0.5,#Cut off Log2FC, automatically 2
                pointSize = 3,
                labSize = 0.2,
...
#Run the functions
GSEA1 <- read.csv("OutputData/Proteomics/GSEA_result_KEGG-Hallmark-Reactome-Biocarta-ATF4_TGvWT_.csv")
  
DoVolcano_1(InputData = GSEA1, 
                  OutputfileName="TGvWT")
```
\
2. GO-terms:\
...
```
```{r, warning= FALSE, message=FALSE}
DoVolcano_2 <- function(InputData, OutputfileName){
  gsea_result2 <- InputData
  #Plot
  Volcano2 <- separate(gsea_result2,"pathway", into = c("GO","signature","rest"), sep= "_", remove=FALSE)%>% 
...
#Run the functions
GSEA2 <- read.csv("OutputData/Proteomics/GSEA_result_GO-terms_TGvWT_.csv")

DoVolcano_2(InputData = GSEA2, 
                  OutputfileName="TGvWT")
```
...
VolcanoPlot_Proteomics <- function(Signature_Merge_Prot, Signature, OutputPlotName, NES, p.adj){
  ...
  VolcanoPlot1 <-EnhancedVolcano (Signature_Merge_Prot,
                lab = Signature_Merge_Prot$Gene,#Metabolite name
                x = "log2FoldChange",#Log2FC
                y = "pval",#p-value or q-value
                xlab = bquote(~Log[2]~ "FC"),
                ...
                colAlpha = 0.5,
                title="Proteomics: D7_TGvWT",
                subtitle = (file=paste("", OutputPlotName, " (NES=", NES, "p.val=", p.adj, ")",sep="")),
                caption = paste0("total = ", nrow(Signature_Merge_Prot), " genes of ", nrow(Signature), " genes in pathway"),
                #xlim = c(-3.5,2.5),
                ylim = c(0,15),
                #drawConnectors = TRUE,
  ...
  ggsave(file=paste("Figures/Proteomics/VolcanoPlot_Proteomics_TGvWT_p.val", OutputPlotName, ".pdf", sep="_"), plot=VolcanoPlot1, width=10, height=8)
   plot(VolcanoPlot1)
}
